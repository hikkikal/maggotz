...
<head>
    ...
    <title>Register Smart Maggot</title>
    ...
    <svg width="0" height="0" class="absolute">
        <defs>
            <clipPath id="wave-clip" clipPathUnits="objectBoundingBox">
                <path d="M 0,0 L 0.82,0 C 0.95,0.25 0.75,0.45 0.88,0.70 C 0.95,0.85 0.80,0.95 0.75,1 L 0,1 Z" />
            </clipPath>
            <clipPath id="wave-clip-mobile" clipPathUnits="objectBoundingBox">
                <path d="M 0,0 L 1,0 L 1,0.85 C 0.75,1 0.25,0.70 0,0.9 Z" />
            </clipPath>
        </defs>
    </svg>

    {{-- Card utama — lebih tinggi dari login karena ada lebih banyak field --}}
    <div class="relative w-full max-w-[420px] md:max-w-[960px] md:h-[620px] bg-[#0D3B16] rounded-[24px] md:rounded-[32px] overflow-hidden shadow-[0_30px_70px_-15px_rgba(13,59,22,0.3)] flex flex-col md:flex-row card-animation z-10">
        ...
        <div class="w-full md:w-[45%] bg-transparent flex flex-col justify-center items-center lg:px-14 md:px-10 px-8 pb-10 pt-4 md:py-0 z-20 md:my-auto">
            <div class="w-full max-w-[300px] text-white">
                ...
                <h1 class="text-2xl md:text-[28px] font-bold tracking-wider text-center uppercase mb-5 md:mb-6 text-white">
                    REGISTER
                </h1>

                {{-- Error validasi --}}
                @if ($errors->any())
                    <div class="mb-4 bg-red-500/20 border border-red-400/50 rounded-xl px-4 py-3">
                ...
                <form method="POST" action="{{ route('register') }}" class="space-y-3.5">
                    ...
                    {{-- Nama --}}
                    <div>
                        <label class="block mb-1.5 text-xs font-normal text-white/85 tracking-wide">Nama Lengkap</label>
                    ...
                    {{-- Email --}}
                    <div>
                        <label class="block mb-1.5 text-xs font-normal text-white/85 tracking-wide">Email</label>
                        <div class="group flex items-center bg-white focus-within:ring-2 focus-within:ring-[#EAD39D] rounded-xl px-4 transition-all duration-200 shadow-sm">
                            <i class="fa-regular fa-envelope text-[#0D3B16]/60 group-focus-within:text-[#0D3B16] transition-colors mr-3 text-sm"></i>
                    ...
                    {{-- Konfirmasi Password --}}
                    <div>
                        <label class="block mb-1.5 text-xs font-normal text-white/85 tracking-wide">Konfirmasi Password</label>
                        <div class="group flex items-center bg-white focus-within:ring-2 focus-within:ring-[#EAD39D] rounded-xl px-4 transition-all duration-200 shadow-sm">
                            <i class="fa fa-lock text-[#0D3B16]/60 group-focus-within:text-[#0D3B16] transition-colors mr-3 text-sm"></i>
                            <input type="password" name="password_confirmation" id="passwordConfirm" required
                                ...
                                autocomplete="new-password"
                                class="w-full h-[44px] bg-transparent outline-none text-sm text-[#0D3B16] placeholder-[#0D3B16]/40 font-medium">
                            <button type="button" id="toggleConfirm" class="focus:outline-none flex items-center">
                                <i id="eyeIconConfirm" class="fa fa-eye-slash text-[#0D3B16]/50 hover:text-[#0D3B16] transition-colors ml-2 cursor-pointer text-sm"></i>
                            </button>
                        </div>
                    </div>
                    ...
                    {{-- Link ke Login --}}
                    <p class="text-center text-xs text-white/60 pt-1">
                        Sudah punya akun?
                        <a href="{{ route('login') }}" class="text-[#EAD39D] hover:text-[#e2c68a] font-semibold transition-colors">
                            Login di sini
                        </a>
                    </p>
