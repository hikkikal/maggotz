...
<div class="relative z-10 w-full font-sans select-none space-y-4 md:space-y-6">
    ...
    <div class="bg-white/70 backdrop-blur-md rounded-2xl shadow-[0_12px_30px_-10px_rgba(13,59,22,0.08)] border border-white/50 relative overflow-hidden">
        ...
        <div class="px-4 md:px-6 py-4 md:py-6 flex items-center gap-4">
            <div class="w-10 h-10 rounded-xl bg-[#0D3B16]/10 flex items-center justify-center text-[#0D3B16] shrink-0">
                <i class="fa-solid fa-pen-to-square text-lg"></i>
            </div>
            <div>
                <h1 class="text-lg md:text-2xl font-extrabold text-[#0D3B16] uppercase">Edit Data Maggot</h1>
                <p class="text-xs text-gray-400 mt-0.5">Perbarui data pencatatan budidaya</p>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white/60 backdrop-blur-md rounded-2xl p-4 md:p-6 border border-white/50">
        <form action="{{ route('maggot-record.update', $record->id) }}" method="POST" class="space-y-4 md:space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div class="space-y-1.5">
                    <label class="block text-xs font-bold text-[#0D3B16] uppercase tracking-wider">
                        <i class="fa-regular fa-calendar text-gray-400 mr-1"></i> Tanggal <span class="text-red-400">*</span>
                    </label>
                    <input type="date" name="tanggal" required value="{{ old('tanggal', $record->tanggal->format('Y-m-d')) }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="device_id"
                        ...
                        <option value="{{ $device->id }}" {{ old('device_id', $record->device_id) == $device->id ? 'selected' : '' }}>
                            {{ $device->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative flex items-center">
                        <input type="number" step="0.1" min="0" name="sampah_masuk" required
                            value="{{ old('sampah_masuk', $record->sampah_masuk) }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative flex items-center">
                        <input type="number" step="0.1" min="0" name="berat_biomassa" required
                            value="{{ old('berat_biomassa', $record->berat_biomassa) }}"
                            class="w-full text-xs font-semibold text-gray-600 border border-gray-200 bg-white/80 rounded-xl pl-4 pr-12 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#0D3B16] transition-all" />
                        <span class="absolute right-3 text-[10px] font-bold text-gray-400 bg-gray-100 px-2 py-0.5 rounded border border-gray-200">kg</span>
                    </div>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <div class="relative flex items-center">
                        <input type="number" step="0.1" min="0" name="hasil_panen"
                            value="{{ old('hasil_panen', $record->hasil_panen) }}"
                            class="w-full text-xs font-semibold text-gray-600 border border-gray-200 bg-white/80 rounded-xl pl-4 pr-12 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#0D3B16] transition-all" />
                        <span class="absolute right-3 text-[10px] font-bold text-gray-400 bg-gray-100 px-2 py-0.5 rounded border border-gray-200">kg</span>
                    </div>
                </div>
            ...
            <div class="space-y-1.5">
                ...
                <textarea name="keterangan" rows="3"
                    class="w-full text-xs font-medium text-gray-600 border border-gray-200 bg-white/80 rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[#0D3B16] transition-all resize-none">{{ old('keterangan', $record->keterangan) }}</textarea>
            </div>
